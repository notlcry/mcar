...
class MockLOBOROBOT:
    ...
    def t_up(self, speed, t_time):
        ...
        time.sleep(min(t_time, 0.2))  # 限制测试时间
...
class SimplePersonalityManager:
    ...
    def __init__(self, robot_controller: MockLOBOROBOT):
        ...
        self.max_continuous_movement_time = 3.0  # 测试用较短时间
        self.movement_start_time = None
        
        self._initialize_movement_patterns()
        logger.info("简化个性管理器初始化完成")
                ...
                for action_data in actions:
                    ...
                    if action_name in ["t_up", "t_down", "moveLeft", "moveRight", 
                        ...
                        if "t_time" not in action_params:
                            ...
                            action_params["t_time"] = min(time_per_action, 0.3)  # 测试用限制
                    
                    self._execute_single_action(action_name, action_params)
                    
                    if pattern.pause_between > 0:
                        time.sleep(min(pattern.pause_between, 0.1))  # 测试用限制
                
                if rep < pattern.repetitions - 1:
                    time.sleep(min(pattern.pause_between * 2, 0.2))
    ...
    def handle_conversation_command(self, command: str, emotion: EmotionType) -> bool:
        ...
        if matched_action:
            ...
            if action_name in ["spin_happy", "dance", "follow_me"]:
    ...
    def _execute_special_action(self, action_name: str, emotion: EmotionType, params: Dict) -> bool:
        ...
        try:
            if action_name == "spin_happy":
                ...
                speed = 50
                self.robot.turnLeft(speed, 0.15)
                self.robot.turnRight(speed, 0.15)
                self.robot.t_up(speed, 0.2)
                self.robot.t_stop(0.1)
                
            elif action_name == "follow_me":
                # 跟随动作
                speed = 45
                self.robot.t_up(speed, 0.5)
                self.robot.t_stop(0.1)
                self.robot.t_up(speed, 0.5)
                self.robot.t_stop(0.1)
...
def test_emotional_movements():
    ...
    personality_manager = SimplePersonalityManager(mock_robot)
...
def test_requirements_validation():
    ...
    print("需求 3.1: 当用户说出移动指令如'我们走吧'时，机器人应前进并显示兴奋表情")
    mock_robot.clear_action_log()
    success = personality_manager.handle_conversation_command("我们走吧", EmotionType.EXCITED)
    print(f"结果: {'✓ 通过' if success else '✗ 失败'}")
    
    print("\n需求 3.2: 当用户要求机器人'转个圈'时，机器人应原地旋转并显示开心动画")
    mock_robot.clear_action_log()
    success = personality_manager.handle_conversation_command("转个圈", EmotionType.HAPPY)
    print(f"结果: {'✓ 通过' if success else '✗ 失败'}")
    
    print("\n需求 3.3: 当AI判断应表达不同意时，机器人应后退并显示消极表情")
    mock_robot.clear_action_log()
    success = personality_manager.execute_emotional_movement(EmotionType.SAD, 0.6)
    action_log = mock_robot.get_action_log()
    has_backward = any(action['action'] == 't_down' for action in action_log)
    print(f"结果: {'✓ 通过' if success and has_backward else '✗ 失败'}")
    
    print("\n需求 3.4: 当对话话题令人兴奋时，机器人应使用更快、更生动的动作")
    mock_robot.clear_action_log()
    success = personality_manager.execute_emotional_movement(EmotionType.EXCITED, 0.9)
    action_log = mock_robot.get_action_log()
    speeds = [action['params'].get('speed', 0) for action in action_log if 'speed' in action['params']]
    avg_speed = sum(speeds) / len(speeds) if speeds else 0
    print(f"平均速度: {avg_speed:.1f}")
    print(f"结果: {'✓ 通过' if success and avg_speed > 60 else '✗ 失败'}")
